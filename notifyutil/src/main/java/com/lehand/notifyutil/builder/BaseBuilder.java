package com.lehand.notifyutil.builder;
...
import com.lehand.notifyutil.NotifyUtil;
...
public class BaseBuilder {
    ...
    //android 8.0适配
    public String channelId = "channel_1";
    public String channalName = "channel_name_1";

    public BaseBuilder setChannalId(String channalId) {
        this.channelId = channalId;
        return this;
    }

    public BaseBuilder setChannalName(String channalName) {
        this.channelId = channalName;
        return this;
    }

    public void build() {
        if (Build.VERSION.SDK_INT >= 26) {
            NotifyUtil.createNotificationChannel(channelId, channalName);
        }
        cBuilder = new NotificationCompat.Builder(NotifyUtil.context, channelId);
        cBuilder.setContentIntent(contentIntent);// 该通知要启动的Intent
        /*图标显示问题，部分资源可能导致程序直接崩溃，使用小图标大小不可过大(24dp左右)，不然会显示不出来*/
        if (smallIcon > 0) {
            try {
                cBuilder.setSmallIcon(smallIcon);// 设置顶部状态栏的小图标
            }catch (Exception e){
                Log.e("Exception",e.getMessage());
                cBuilder.setSmallIcon(android.R.drawable.stat_notify_more);
                e.printStackTrace();
            }
        }
        if (bigIcon > 0) {
        ...
        Log.e("TAG", "smallIcon = " + smallIcon);
        Log.e("TAG", "bigIcon = " + bigIcon);
        Log.e("TAG", "contentTitle = " + contentTitle);
        Log.e("TAG", "contentText = " + contentText);
